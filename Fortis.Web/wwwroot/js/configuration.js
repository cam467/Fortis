var BucApiSettings = (function() {
    ...
    var _url = '/settings/',
        ...
        startUp = function() {
            ...
            $(document).on('click', '.page-sidebar-menu a', function(e) {
            ...
            }).on('click', 'table[id*=schedules] div.editbuttons i.fa-play-circle', function(e) {
                ...
                if (isdelete) {
                    ...
                    $.ajax({
                        method: 'GET',
                        url: _url + (isedit ? "editsetting/" + controlkey : "addsetting"),
                        success: function(d) {
                            $('#modaladdsetting').find('div.modal-header span').html(isedit ? "Edit Setting" : "Add Setting").end().find('div.modal-body').html(d).end().find('#savesetting').off().on('click', function() {
                                ...
                                $.ajax({
                                    ...
                                    url: _url + (isedit ? 'editsetting' : 'addsetting'),
                                    dataType: 'json',
                                    contentType: 'application/json',
