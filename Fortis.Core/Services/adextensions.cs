namespace Fortis.Core.Services
{
    ...
    public class ADExtensions : IADExtensions
    {
        ...
        public List<User> GetAllADUsersForGroups(List<string> _groups, string _ouroot)
        {
            ...
            search.PropertiesToLoad.Add("userAccountControl"); // smtp mail address
            ...
            try
            {
                ...
                for (int i = 0; i < rpc.Count; i++)
                {
                    try
                    {
                        bool uac = rpc[i].Properties["userAccountControl"] != null ? !Convert.ToBoolean((int)rpc[i].Properties["userAccountControl"][0] & 0x0002) : true;
                        users.Add(new User { first_name = rpc[i].Properties["givenName"][0].ToString(), last_name = rpc[i].Properties["sn"][0].ToString(), email = rpc[i].Properties["mail"][0].ToString(), status = uac.ToString() });
                    }
                    catch
                    {
                    }
